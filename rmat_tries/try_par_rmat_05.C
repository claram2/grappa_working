...
int display_size = 20;
  ...
  typedef property<position_t, Point, property <diff_t, Point> > VertexProperty;
...
//  Point2* extent = new Point2(1.0, 1.0);

  grid_force_pairs<topology, PositionMap> my_grid(*my_h, position, g);
  //std::cout << "Origin = " << (*origin)[0] << "," << (*origin)[1] << " Extent = " << (*extent)[0] << "," << (*extent)[1] << std::endl;

  boost::graph::distributed::fruchterman_reingold_force_directed_layout(g, position, Point2(0.0, 0.0), Point2(1.0, 1.0), square_distance_attractive_force(), square_distance_repulsive_force(), all_force_pairs(), linear_cooling<double>(10), displacement);
