...
double distance;
 for(tie(e_i, e_end) = edges(g); e_i != e_end; ++e_i){
  ...
  outfile << index[source(*e_i, g)] << " -- " << index[target(*e_i, g)] << " [color = \"" << ((int)(distance*1000))*0.001 << " " << (((int)(distance*1000000))% 1000)*0.001 << " " << (((int)(distance*1000000000))% 1000)*0.001 << "\"]" << std::endl;
 }
 outfile << "}" << std::endl;
