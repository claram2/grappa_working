...
int main(int argc, char* argv[])
{
  ...
  typedef property<position_t, heart_topology<>::point_type> PositionProperty;
  ...
  heart_topology<>* my_h = new heart_topology<>(gen);
 ...
 outfile << "graph [bgcolor = \"black\", fontcolor=\"white\" size=\"6,6\" ratio=fill]" << std::endl;
 outfile << "node [width=0.25, height=0.25, fontsize=8.0 fontcolor=\"white\"]" << std::endl;
...
double distance;
 for(tie(e_i, e_end) = edges(g); e_i != e_end; ++e_i){
  ...
  outfile << std::setfill ('0') << std::setw(2) << std::hex << (int)(((int)(distance*100))*0.001*255); 
  outfile << std::setfill ('0') << std::setw(2) << std::hex << (int)((((int)(distance*100000))% 1000)*0.001*255);
  outfile << std::setfill ('0') << std::setw(2) << std::hex << (int)((((int)(distance*100000000))% 1000)*0.001*255);
